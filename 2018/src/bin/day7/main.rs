const REGEX_STR: &str =
    "Step (?P<need>[A-Z]) must be finished before step (?P<node>[A-Z]) can begin.";
...
fn part1(input: &str) -> Result<String> {
    ...
    fn step(dep_map: &mut DepMap) -> Result<u8> {
        ...
        let (&next, _) = dep_map
            ...
            .expect("Couldn't find empty needs");
        dep_map.remove(&next);
    ...
    for node in b'A'..=b'Z' {
        // Ensure every node has an entry, even if blank
...
fn part2(input: &str, worker_count: usize) -> Result<usize> {
    ...
    fn tick(dep_map: &mut DepMap, workers: &mut Vec<Worker>, todo: &mut Vec<u8>) -> Result<usize> {
        ...
        if dep_map.is_empty() {
            let final_time = workers
                .iter()
                .map(|w| if let Worker::Working(_, t) = w { *t } else { 0 })
                .max()?;
